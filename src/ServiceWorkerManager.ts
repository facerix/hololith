...
export class ServiceWorkerManager {
  ...
  async register(): Promise<ServiceWorkerRegistration | null> {
    if (!('serviceWorker' in navigator)) {
      console.warn(`[Hololith] Service Workers not supported in this browser`);
      return null;
    }

    const existingRegistration = await navigator.serviceWorker.getRegistration();
    if (existingRegistration) {
      console.log(`[Hololith] Service Worker already registered`);
      this.#registration = existingRegistration;
      this.#isRegistered = true;

      this.#checkForMultipleWorkers().catch(error => {
        console.warn(`[Hololith] Error checking for multiple workers:`, error);
      });
    ...
    if (this.#isRegistered) {
      console.log(`[Hololith] Service Worker already registered in this instance`);
      return this.#registration;
    }
    ...
    try {
      ...
      console.log(`[Hololith] Service Worker registered successfully:`, this.#registration.scope);

      this.#setupUpdateListeners();

      return this.#registration;
    } catch (error) {
      console.error(`[Hololith] Service Worker registration failed:`, error);
      return null;
    }
  }
  ...
  async #checkForMultipleWorkers(): Promise<void> {
    ...
    if ((active && waiting) || (active && installing) || (waiting && installing)) {
      console.warn(`[Hololith] Multiple service workers detected:`, {
      ...
      if (waiting && controller && waiting !== controller && !this.#isUpdating) {
        try {
          ...
          if (activeVersion === waitingVersion) {
            console.log(`[Hololith] Waiting worker is same version as active. Auto-activating...`);
            await this.skipWaiting(waiting);
            return;
          }
        } catch (error) {
          console.warn(`[Hololith] Could not verify versions, proceeding normally:`, error);
        }
      }
    }
  }
  ...
  #setupUpdateListeners(): void {
      ...
      if (reg?.waiting && navigator.serviceWorker.controller && !this.#isUpdating) {
        ...
        try {
          ...
          if (currentVersion === latestVersion) {
            console.log(`[Hololith] Waiting worker is same version, skipping notification`);
            return;
          }
        } catch (error) {
          console.warn(`[Hololith] Could not verify versions:`, error);
        }

        console.log(`[Hololith] Found waiting service worker from previous session`);
    ...
    this.#registration.addEventListener('updatefound', () => {
      ...
      console.log(`[Hololith] New service worker installing...`);

      const handleStateChange = (): void => {
        console.log(`[Hololith] Service worker state changed to: ${newWorker.state}`);
          ...
          this.getLatestVersion()
            .then(latestVersion => {
              return this.getVersion().then(currentVersion => {
                if (currentVersion === latestVersion) {
                  console.log(`[Hololith] New worker is same version, skipping notification`);
                  return false;
                }
                return true;
              });
            ...
            .then(shouldShow => {
              if (shouldShow) {
                console.log(
                  `[Hololith] New service worker available. Consider refreshing the page.`
                );
            ...
            .catch(error => {
              console.warn(`[Hololith] Could not verify version, showing notification:`, error);
              console.log(`[Hololith] New service worker available. Consider refreshing the page.`);
              this.#dispatchUpdateEvent(newWorker);
              newWorker.removeEventListener('statechange', handleStateChange);
            });
        }
  ...
  #dispatchUpdateEvent(pendingWorker: ServiceWorker | null): void {
    if (this.#isUpdating) {
      console.log(`[Hololith] Update already in progress, skipping notification`);
      return;
    }
  ...
  async checkForUpdates(): Promise<void> {
    if (!this.#registration) {
      console.warn(`[Hololith] Cannot check for updates: no registration`);
      return;
    }

    try {
      console.log(`[Hololith] Manually checking for service worker updates...`);
      await this.#registration.update();

      setTimeout(() => {
        if (this.#registration?.waiting && navigator.serviceWorker.controller) {
          console.log(`[Hololith] Update check found waiting service worker`);
          this.#dispatchUpdateEvent(this.#registration.waiting);
        }
      }, 100);
    } catch (error) {
      console.error(`[Hololith] Failed to check for updates:`, error);
    }
  }
  ...
  async getVersion(): Promise<string | null> {
    ...
    try {
      ...
      console.error(`[Hololith] Failed to get service worker version:`, error);
      return null;
    }
  }
  ...
  async getLatestVersion(): Promise<string | null> {
    ...
    try {
      ...
      console.error(`[Hololith] Failed to get latest service worker version:`, error);
      return null;
    }
  }
  ...
  async skipWaiting(worker: ServiceWorker | null = null): Promise<void> {
    ...
    if (!this.#registration || !targetWorker) {
      console.warn(`[Hololith] No waiting service worker to skip waiting`);
      return;
    }

    this.#dispatchUpdateProgress('Sending activation signal...');
    console.log(`[Hololith] Sending SKIP_WAITING message to service worker`);
    ...
    return new Promise<void>(resolve => {
      ...
      const handleControllerChange = (): void => {
        ...
        if (navigator.serviceWorker.controller) {
          ...
          console.log(`[Hololith] New service worker is now controlling the page`);
          setTimeout(() => {
            if (!this.#registration?.waiting || this.#registration.waiting !== targetWorker) {
              console.log(`[Hololith] Old waiting worker has been terminated`);
              this.#dispatchUpdateProgress('Preparing to reload...');
            } else {
              console.warn(`[Hololith] Warning: Waiting worker still exists`);
          ...
          setTimeout(() => {
            if (navigator.serviceWorker.controller) {
              console.log(`[Hololith] New service worker is now controlling the page (delayed)`);
              this.#dispatchUpdateProgress('Preparing to reload...');
              if (!resolved) {
                resolved = true;
                resolve();
              ...
              console.warn(`[Hololith] No controller after skipWaiting, resolving anyway`);
      ...
      const handleMessage = (event: MessageEvent): void => {
        if (event.data && event.data.type === 'SW_ACTIVATED') {
          console.log(`[Hololith] Service worker confirmed activation: ${event.data.version}`);
      ...
      setTimeout(() => {
        if (!resolved) {
          console.log(`[Hololith] Skip waiting timeout, proceeding with reload`);
  ...
  async clearAllCaches(): Promise<void> {
    if (!('caches' in window)) {
      console.warn(`[Hololith] Cache API not supported`);
      return;
    }

    try {
      const cacheNames = await caches.keys();
      const appCaches = cacheNames.filter(name => name.startsWith('hololith-cache-'));

      console.log(`[Hololith] Clearing ${appCaches.length} cache(s):`, appCaches);

      await Promise.all(appCaches.map(cacheName => caches.delete(cacheName)));

      console.log(`[Hololith] Successfully cleared all caches`);

      if (this.#registration) {
        await this.#registration.unregister();
        console.log(`[Hololith] Service worker unregistered`);
      ...
      console.error(`[Hololith] Failed to clear caches:`, error);
      throw error;
    }
  }
  ...
  async handleUpdateNow(pendingWorker: ServiceWorker): Promise<void> {
    if (this.#isUpdating) {
      console.log(`[Hololith] Update already in progress`);
      return;
    }

    this.#isUpdating = true;
    console.log(`[Hololith] Handling update now request`);
    ...
    try {
      ...
      if (!navigator.serviceWorker.controller) {
        ...
        console.warn(
          `[Hololith] No service worker controller after skipWaiting, waiting a bit longer...`
        );
      ...
      if (this.#registration?.waiting && this.#registration.waiting === pendingWorker) {
        this.#dispatchUpdateProgress('Waiting for old worker to terminate...');
        console.warn(
          `[Hololith] Warning: Waiting worker still exists after skipWaiting. Waiting longer...`
        );
        await new Promise<void>(resolve => setTimeout(resolve, 1000));

        if (this.#registration?.waiting === pendingWorker) {
          console.error(
            `[Hololith] Error: Waiting worker still exists. This may cause multiple workers.`
          );
        }
      }

      this.#dispatchUpdateProgress('Reloading page...');
      console.log(`[Hololith] Reloading page to use new service worker...`);
      window.location.reload();
    } catch (error) {
      console.error(`[Hololith] Failed to update service worker:`, error);
